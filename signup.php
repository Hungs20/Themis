    ...
    if ($username) {
        foreach ($row as $r) {
         ...
         if ($found == 1) {
            ...
            $rowAttr = $doc->createAttribute('ss:AutoFitHeight');
            $row->appendChild($rowAttr);
            $rowAttrVal = $doc->createTextNode('0');
            $rowAttr->appendChild($rowAttrVal);

            //==========ID========//
            $cell = $doc->createElement('Cell');
            $cellAttr = $doc->createAttribute('ss:StyleID');
            $cell->appendChild($cellAttr);
            $cellAttrVal = $doc->createTextNode('s63');
            $cellAttr->appendChild($cellAttrVal);

            $data = $doc->createElement('Data');
            $dataAttr = $doc->createAttribute('ss:Type');
            $data->appendChild($dataAttr);
            $dataAttrVal = $doc->createTextNode('Number');
            $dataAttr->appendChild($dataAttrVal);
            ...
            $dataAttr = $doc->createAttribute('ss:Type');
            $data->appendChild($dataAttr);
            $dataAttrVal = $doc->createTextNode('String');
            $dataAttr->appendChild($dataAttrVal);

            $data->nodeValue=$username;
            $cell->appendChild($data);
            $row->appendChild($cell);
            ...
            $doc->save("data/account.xml");

            header("Location: login.php?callback=signup");
        }
    }
...
<title>Đăng kí · Chấm bài trực tuyến</title>
...
<input type=text id=fullname name=fullname class=email maxlength=256>
</div>
<div class="inputGroup inputGroup1">
...
<div class="inputGroup inputGroup1 helper"> <?php if (isset($_GET['err'])) echo "<font color = 'red'><b><i>Tên đăng nhập đã tồn tại!</i></b></font>"; ?> </div>
<div class="inputGroup inputGroup3">
<button id=login>Đăng kí</button>
</div>
<div class="inputGroup inputGroup1 helper"><center><a href="login.php"><label>Quay lại đăng nhập</label></a></center></div>
</form>
